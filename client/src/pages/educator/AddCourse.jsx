...
const AddCourse = () => {
      ...
      setChapters(
        chapters.map((chapter) => {
          if (chapter.chapterId === chapterId) {
            // Create a new array excluding the removed lecture
            return {
              ...chapter,
  ...
  const addLecture = () => {
    setChapters(
      chapters.map((chapter) => {
        if (chapter.chapterId === currentChapter) {
          ...
          // Return a new chapter object with the new lecture appended
          return {
            ...chapter,
    ...
    // Reset popup and form state after adding
    setShowPopUp(false);
    setLectureDetail({
      lectureTitle: '',
      lectureDuration: '',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // You can access the quill HTML content here via: 
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:pb-0 p-4 pt-8 pb-0'>
      <form className='flex flex-col gap-4 max-w-md w-full text-gray-500' onSubmit={handleSubmit}>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            ...
            <input
              ...
              type="number"
              placeholder='0'
              className='outline-none md:py-2.5 px-3 py-2 w-28 rounded border border-gray-500'
              required
            />
          </div>
        ...
        <div className="w-full">
          ...
          <div
            className='flex justify-center items-center bg-blue-100 text-blue-800 p-2 rounded-lg cursor-pointer font-medium'
          ...
          {showPopUp && (
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50'>
              <div className='bg-white text-gray-700 p-6 rounded relative w-full max-w-sm'>
                ...
                <button
                  type="button" 
                  className='w-full bg-blue-500 hover:bg-blue-600 transition-colors text-white py-2 rounded font-medium'
                  onClick={addLecture}
                >
                  Add
                </button>
                ...
                <img
                  ...
                  src={assets.cross_icon}
                  alt="close"
                  className='absolute top-4 right-4 w-3 cursor-pointer'
                />
              </div>
            </div>
          )}
