    ...
    <>
      <div className='p-4 sm:p-10 flex flex-row md:grid md:grid-cols-2 gap-10 md:px-36'>
        ...
        <div className='md:mt-10'>
          {playerData ? (
            <div>
              <YouTube videoId={playerData.lectureUrl.includes("v=") ? playerData.lectureUrl.split("v=")[1].split("&")[0] : playerData.lectureUrl.split('/').pop().split('?')[0]} opts={{
                    playerVars: { autoplay: 1 }
                  }} iframeClassName='w-full aspect-video' />
                  <div className='flex justify-between items-center mt-1'>
                    <p>{playerData.chapter}.{playerData.lecture} {playerData.lectureTitle}</p>
                    <button onClick={() => markLectureCompleted(playerData.lectureId)} className='text-blue-600 cursor-pointer'>{progressData && progressData.lectureCompleted.includes(playerData.lectureId) ?'Completed':'Mark Completed'}</button>
                  </div>
            </div>
